// client-mobile/src/shared/components/ui/TiyuMascot.jsx
import { forwardRef, useEffect, useImperativeHandle, useRef } from 'react';
import { GLView } from 'expo-gl';
import { Renderer } from 'expo-three';
import * as THREE from 'three';

// performance.now() no siempre existe en el motor JS de RN (Hermes), usamos
// un fallback a Date.now() por si acaso.
const now = () => (typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now());

/**
 * Mascota 3D de Tiyú (Three.js vía expo-gl/expo-three) para React Native.
 * Es el equivalente mobile de client-admin/src/shared/components/ui/TiyuMascot.jsx,
 * misma geometría y misma animación de saludo, adaptada a un contexto GL de Expo
 * en lugar de un <canvas> del DOM.
 *
 * Expone sayHello() vía ref para disparar el saludo desde donde lo necesites
 * (ej. justo cuando el usuario inicia sesión).
 *
 * Props:
 *  - size: tamaño en px del view (cuadrado)
 *  - greet: si es true, hace la animación de saludo automáticamente al montarse
 *  - style: estilos extra para el GLView (opcional)
 */
export const TiyuMascot = forwardRef(({ size = 90, greet = false, style }, ref) => {
    const sayHelloFnRef = useRef(null);
    // Si alguien llama sayHello() ANTES de que la escena 3D termine de montarse
    // (el contexto GL tarda unos frames en crearse), guardamos la intención aquí
    // y la disparamos apenas esté lista.
    ...
    const onContextCreate = async (gl) => {
        ...
        renderer.setClearColor(0x000000, 0); // transparente, se integra con el fondo de la app
        ...
        const startTime = now();
        sayHelloFnRef.current = () => {
            waveState = { startTime: (now() - startTime) / 1000, duration: 2.0 };
        };

        // Si nos pidieron saludar mientras la escena todavía se montaba, lo hacemos ahora.
