// src/features/admin/screens/AdminContentScreen.jsx
...
const EMPTY_FORM = { title: '', description: '', type: '', category: '', url: '' };
...
export default function AdminContentScreen() {
    ...
    const handleCreate = async () => {
        ...
        setSaving(true);
        try {
            await healthyClient.post('/contents', form);
            setModalOpen(false);
            setForm(EMPTY_FORM);
            fetchContents();
        } catch (err) {
            Alert.alert('Error', err.response?.data?.message || 'No se pudo crear el contenido');
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = (item) => {
        Alert.alert('Eliminar', `¿Eliminar "${item.title}"? Esta acción no se puede deshacer.`, [
            { text: 'Cancelar', style: 'cancel' },
            {
                ...
                onPress: async () => {
                    try {
                        await healthyClient.delete(`/contents/${item._id}`);
                        setContents((prev) => prev.filter((c) => c._id !== item._id));
                    } catch (err) {
    ...
    return (
        <View style={styles.flex}>
            <FlatList
                ...
                refreshing={loading}
                ListEmptyComponent={<EmptyState icon="article" title="Sin contenido publicado" />}
                renderItem={({ item }) => (
                    <Card style={styles.card}>
                        ...
                        <View style={styles.tagsRow}>
                            ...
                            <Text style={[styles.tag, styles.tagAlt]}>{item.category || 'GENERAL'}</Text>
                        </View>
                    </Card>
                )}
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalCard}>
                        <ScrollView showsVerticalScrollIndicator={false}>
                            <Text style={styles.modalTitle}>Nuevo contenido</Text>
                            ...
                            />
                            <Input
                                label="URL (si es video/enlace)"
                                value={form.url}
                                onChangeText={(v) => setForm((f) => ({ ...f, url: v }))}
                                autoCapitalize="none"
                            />

                            <Button title="Guardar" onPress={handleCreate} loading={saving} style={{ marginTop: SPACING.md }} />
                            <Button
                                title="Cancelar"
                                variant="secondary"
